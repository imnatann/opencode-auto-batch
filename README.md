...
`opencode-auto-batch` is a standalone OpenCode plugin package for people who want one default `auto` agent, aggressive safe parallelism, and visible multi-task execution.

It is built on top of the `oh-my-opencode` plugin architecture, but published as its own standalone package and repo.

## What It Adds

This standalone plugin is tuned for OpenCode users who want an `auto` front-door agent, aggressive `Kerjain Semua` parallel execution, and multiple active todos showing as `in_progress` at the same time.

- Recommended repo name: `opencode-auto-batch`
- Install guide: `docs/guide/standalone-installation.md`
- Example configs: `docs/examples/opencode.json`, `docs/examples/oh-my-opencode.json`
- GitHub: `https://github.com/imnatann/opencode-auto-batch`
...
```
Install and configure opencode-auto-batch by following the instructions here:
https://raw.githubusercontent.com/imnatann/opencode-auto-batch/main/docs/guide/standalone-installation.md
```

Or read the [Standalone Installation Guide](docs/guide/standalone-installation.md).
...
```bash
curl -s https://raw.githubusercontent.com/imnatann/opencode-auto-batch/main/docs/guide/standalone-installation.md
```
