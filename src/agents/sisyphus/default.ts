...
2. **Before starting each step**: \`TaskUpdate(status="in_progress")\` (mark every concurrently active task in_progress when truly parallel; otherwise keep one)
...
2. **Before starting each step**: Mark \`in_progress\` (mark every concurrently active task in_progress when truly parallel; otherwise keep one)
