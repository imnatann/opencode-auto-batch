...
export function formatVersionOutput(info: VersionInfo): string {
  ...
  lines.push("")
  lines.push(color.bold(color.white("opencode-auto-batch Version Information")))
  ...
  switch (info.status) {
    ...
    case "outdated":
      ...
      lines.push(`  ${color.dim("Run:")} ${color.cyan("cd ~/.config/opencode && npm install ../opencode-auto-batch")}`)
      break
    case "local-dev":
